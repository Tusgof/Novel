...
def build_parser() -> argparse.ArgumentParser:
    ...
    report_plan_p = report_subparsers.add_parser("run-plan", help="Generate a read-only run planning report.")
    report_plan_p.add_argument("--run-id", required=True, help="Run ID to inspect.")
    report_plan_p.add_argument("--output", type=Path, default=None, help="Output markdown path.")

    report_cache_p = report_subparsers.add_parser("cache-benchmark", help="Generate a read-only cache benchmark report.")
    report_cache_p.add_argument("--run-id", required=True, help="Run ID to inspect.")
    report_cache_p.add_argument("--output", type=Path, default=None, help="Output markdown path.")

    report_concurrency_p = report_subparsers.add_parser("concurrency-benchmark", help="Generate a read-only concurrency benchmark report.")
    report_concurrency_p.add_argument("--run-id", required=True, help="Run ID to inspect.")
    report_concurrency_p.add_argument("--output", type=Path, default=None, help="Output markdown path.")
...
def cmd_report(args: argparse.Namespace, config) -> int:
    ...
    elif args.report_command == "run-plan":
        result = build_run_plan_report(config=config, run_id=args.run_id, output=args.output)
    elif args.report_command == "cache-benchmark":
        result = build_cache_benchmark_report(config=config, run_id=args.run_id, output=args.output)
    elif args.report_command == "concurrency-benchmark":
        result = build_concurrency_benchmark_report(config=config, run_id=args.run_id, output=args.output)
