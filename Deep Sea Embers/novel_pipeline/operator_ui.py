...
def _operator_quick_links(config: AppConfig, run_id: str | None) -> list[dict[str, str]]:
    ...
    project_brain = control_root / "PROJECT_BRAIN.md"
    implement_plan = control_root / "IMPLEMENT_PLAN.md"
    candidates = [
        ("Project Brain", project_brain if project_brain.exists() else root / "PROJECT_BRAIN.md"),
        ("Implement Plan", implement_plan if implement_plan.exists() else root / "IMPLEMENT_PLAN.md"),
...
def _manual_action_needs_qa_or_recovery(action: str) -> bool:
    lowered = action.strip().lower()
    if not lowered or lowered == "none":
        return False
    if lowered.startswith("resume --run-id ") or lowered.startswith("run --range "):
        return False
    recovery_markers = (
        "failed block",
        "inspect failed",
...
def _employee_status_snapshot(config: AppConfig, status: dict[str, Any], preflight: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    qa_or_recovery_actions = [
        action for action in manual_actions if _manual_action_needs_qa_or_recovery(action)
    ]
    ...
    for index, employee in enumerate(EMPLOYEE_ROSTER):
        ...
        if employee["code"] == "007" and failed_blocks:
            readiness = "warning"
        elif employee["code"] in {"004", "007"} and qa_or_recovery_actions:
            readiness = "warning"
...
def run_provider_smoke_tests(config: AppConfig, *, confirm: bool) -> dict[str, Any]:
    ...
    checked_routes: dict[tuple[str, str], dict[str, Any]] = {}
    for stage, provider_name, model in routes:
        key = (provider_name, model)
        if key not in checked_routes:
            ...
            response = runner.run(
                ProviderRequest(
                    ...
                    timeout_seconds=min(spec.timeout_seconds, smoke_timeout_seconds),
                )
            )
        ...
        checked = checked_routes[key]
        employee = employee_for_stage(stage) or {}
        results.append(
            {
                **checked,
                "stage": stage,
    ...
    for employee in EMPLOYEE_ROSTER:
        ...
        results.append(
            {
                "stage": str(employee["maps_to"][0] if employee["maps_to"] else "local"),
                "provider": "local",
                "model": "built-in",
                "status": "ready",
                "failure_kind": "",
                ...
                "stdout_preview": "local controls available",
                "stderr_preview": "",
...
def generate_operator_report(
    *,
    ...
    if kind == "run-plan":
        if not run_id:
            raise ValueError("run-plan report requires run_id.")
        return build_run_plan_report(config=config, run_id=run_id)
    if kind == "cleanliness":
        if not run_id:
...
<body>
  ...
  <script>
    ...
    function renderActionPreview(targetId, title, command, scopeLines) {
      ...
      if (!command) {
        ...
        wrap.innerHTML = `
          ...
          <div>Action locked until scope is complete.</div>
    ...
    function setActionButtonState(buttonId, enabled, lockedLabel, readyLabel) {
    ...
    function renderActionPreviews() {
      ...
      const batchCommand = batchRun && batchRange
        ...
        : "";
      const batchReady = renderActionPreview(
        "batchPreview",
      ...
      setActionButtonState("batchBtn", batchReady, "Fill Run ID + Range", batchScanOnly ? "Start Scan" : "Start Translation");
      ...
      const resumeCommand = resumeRun && resumeBound
        ...
        : "";
      const resumeReady = renderActionPreview(
        "resumePreview",
        "Bounded resume",
        resumeCommand,
        [
      ...
      setActionButtonState("resumeBtn", resumeReady, "Fill Run ID + Boundary", "Continue To Boundary");
      ...
      const rerunCommand = rerunRun && blockId && stage
        ...
        : "";
      const rerunReady = renderActionPreview(
        "rerunPreview",
        "Rerun block",
        rerunCommand,
        [
      ...
      setActionButtonState("rerunBtn", rerunReady, "Fill Run ID + Block", "Run Rerun-Block");
    }
    ...
    function resolveCurrentBlocker(snapshot) {
      ...
      let nextLabel = "Continue Translation";
      let focusTarget = "operate";

      if (blocking.length) {
        pillClass = "danger";
        title = "Preflight blocking";
        detail = blocking.join(" | ");
        nextLabel = "Open Technical Details";
        focusTarget = "all";
      } else if (failed.length) {
        pillClass = "danger";
        title = "Failed blocks present";
        detail = failed.join(", ");
        nextLabel = "Recover Block";
        ...
        nextLabel = "Continue Carefully";
        focusTarget = "operate";
      }

      return { pillClass, title, detail, nextLabel, focusTarget };
    }
    ...
    function renderCurrentBlocker(snapshot) {
      ...
      wrap.innerHTML = `
        ...
        <div class="next-step-card">
          <div class="next-copy">Do this next: ${escapeHtml(blocker.nextLabel)}</div>
          <button class="mini-action" data-blocker-focus="${escapeHtml(blocker.focusTarget)}">${escapeHtml(blocker.nextLabel)}</button>
        </div>
      `;
      wrap.querySelector("[data-blocker-focus]")?.addEventListener("click", (event) => {
        setDashboardFocus(event.currentTarget.dataset.blockerFocus || "operate");
      });
    }
    ...
    async function loadGlossaryQueue(runId) {
      ...
      if (!items.length) {
        ...
        const completionReady = Boolean(progress.completion_ready);
        const title = totalCandidates === 0
          ? "No glossary terms to review"
          : completionReady
            ? "Glossary review is complete"
            : "No pending glossary terms";
        const detail = totalCandidates === 0
          ? "The scan did not produce effective candidates. Continue only with a bounded translation action."
          : completionReady
            ? "All current candidates have decisions. Continue translation from the Continue Translation task."
            : "The queue is empty for this run. Refresh status before continuing.";
        const nextLabel = completionReady || totalCandidates === 0 ? "Continue Translation" : "Refresh Status";
        const nextFocus = completionReady || totalCandidates === 0 ? "operate" : "glossary";
        wrap.className = "";
        wrap.innerHTML = `
          <div class="surface-card">
            <h4>${escapeHtml(title)}</h4>
            <div class="footer-note">${escapeHtml(detail)}</div>
            <div class="next-step-card" style="margin-top:10px;">
              <div class="next-copy">${escapeHtml(nextLabel)}</div>
              <button class="mini-action" data-empty-glossary-focus="${escapeHtml(nextFocus)}">${escapeHtml(nextLabel)}</button>
            </div>
          </div>
        `;
        wrap.querySelector("[data-empty-glossary-focus]")?.addEventListener("click", (event) => {
          setDashboardFocus(event.currentTarget.dataset.emptyGlossaryFocus || "operate");
        });
    ...
    async function loadGlossarySuggestion(term) {
      ...
      wrap.innerHTML = `
        <div class="stack">
          ...
          <label for="glossaryCustomThaiTerm">Custom Thai term</label>
          <input id="glossaryCustomThaiTerm" placeholder="Optional: type your own Thai term instead of the selected option">
    ...
    async function submitGlossaryDecision(decision) {
      ...
      const customThaiTerm = document.getElementById("glossaryCustomThaiTerm")?.value.trim() || "";
      ...
      const payload = {
        ...
        decision,
        thai_term: decision === "approve" ? (customThaiTerm || selectedOption) : "",
        note,
      };
    ...
    async function runProviderSmoke() {
      ...
      const rows = (data.results || []).map((item) => `
        <li class="mono">
          ${item.employee_code ? `[${escapeHtml(item.employee_code)} ${escapeHtml(item.employee_name || "")}] ` : ""}
          ${escapeHtml(item.stage || "route")}:
