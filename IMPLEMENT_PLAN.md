...
- **Active production work order: Immortality System `ch001-ch150`.** Use sequential five-chapter batches. `ch001-ch011` are complete and repaired `ch011` passed guardrails plus Sentinel `0/0/0/0`. Production/default routing uses Gemini 3.7 Flash and DeepSeek V4 Flash 0731; DeepSeek V4 Pro is removed. The first new-model `ch012-block-004` refinement completed, but reasoning QA hit `WinError 10054`; inspection exposed `ไม่สามารถขั้นสร้างฐาน`, now covered by glossary/guardrail/test prevention. Rerun that block from refinement after explicit resume and do not open `ch016-ch020` until `ch011-ch012` pass. Evidence: `Immortality System/07_Reports/immortality_system_ch011_ch015_production_stop_20260829.md` and `07_Reports/openrouter_model_routing_migration_20260829.md`.
