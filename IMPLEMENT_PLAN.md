...
- **Blocked items**:
  ...
  - Immortality System Pilot resume is blocked at the `林遠` manual glossary decision in `ch1765`. The user must choose one of the three displayed options before the same bounded run can resume. Provider/title fallback behavior is implemented/tested and all 20 sampled title sidecars currently validate.
