...
1. Immortality System: production `ch001-ch150` is approved but currently stopped with `ch011-block-004` pending QA after two OpenRouter reasoning empty-assistant responses. On explicit resume, rerun that block from QA, verify/assemble `ch011`, then rerun `ch012-block-004` from refine and pass both chapters through guardrails/Sentinel. Only then open `ch016-ch020`. Do not copy pilot artifacts into production.
