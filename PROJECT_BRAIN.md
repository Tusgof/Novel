...
- DSE `ch186-ch190` production checkpoint completed and published: run `dse-ch186-ch190-v1` completed `29/29` blocks, current failed blocks none, manual actions none, output guardrails passed, latest scoped Sentinel `0/0/0/0`, and MoonRead now publishes DSE through `ch190`. One provider hang after `ch190-block-005` refinement was recovered by stopping the stale process and resuming from ledger state; no artifact was manually patched. Report: `07_Reports/dse_ch186_ch190_production_checkpoint_20260701.md`.
- DSE `ch191-ch195` production checkpoint completed and published: run `dse-ch191-ch195-v1` completed `29/29` blocks, current failed blocks none, manual actions none, output guardrails passed, final scoped Sentinel `0/0/0/0`, and MoonRead now publishes DSE through `ch195`. Three minor English parenthetical advisories were deterministically removed from final output/formatted artifacts before publication. Report: `07_Reports/dse_ch191_ch195_production_checkpoint_20260701.md`.
...
- Pilot scan-only gates completed: 73 in-sample candidates and 66 out-of-sample candidates. The isolated in-sample run `immortality-libra-v1-insample` has completed 29 blocks across seven sampled chapters (`ch1307`, `ch1765`, `ch2439`, `ch2307`, `ch741`, `ch1424`, `ch1631`) with experiment-only outputs; it is currently blocked at glossary approval for `ch376` / `錢雅` after the configured provider chain produced no safe Thai options. No production output exists and MoonRead remains disabled.
- A bounded pilot title-sidecar preparation attempt stopped on 2026-08-28 when OpenRouter `deepseek/deepseek-v4-flash` returned an empty assistant message during title refinement. That failed batch wrote no sidecars, pilot ledger records, production output, or MoonRead content. Current disk validation finds valid title sidecars for all 20 locked sample chapters. The canonical and experiment-copy title helpers now walk configured fallback routes and return the provider/model actually used; the fallback behavior is covered by `test_title_provider_uses_configured_fallback_after_primary_failure`. Evidence: `Immortality System/07_Reports/immortality_system_libra_pilot_title_provider_stop_20260828.md` and the title-helper regression test.
...
- The first resumed in-sample run completed `ch1307`, `ch1765`, `ch2439`, `ch2307`, and `ch741`; later resumes completed `ch1424` and `ch1631`. A stale `ch1424` title sidecar was rejected because it used `หลิวอีดาว` instead of the approved `หลิวอี้เตา`; rerunning the title provider pipeline after glossary approval regenerated a valid sidecar and final assembly passed.
- The same in-sample run is now stopped at `ch376` term `錢雅`: the configured term-suggestion chain produced no safe Thai options and the final Claude fallback failed because its OAuth session had expired. No `glossary_approved` commit or translation output was written for `ch376`; `ch338` and `ch984` remain pending. Evidence: `Immortality System/07_Reports/immortality_system_libra_pilot_term_provider_stop_20260828.md` and `Immortality System/04_Work/_experiments/libra_pilot_immortality_system_v1/06_Logs/run_ledger.jsonl`.
- Incident reports: `Immortality System/07_Reports/immortality_system_libra_pilot_provider_stop_20260828.md`, `Immortality System/07_Reports/immortality_system_libra_pilot_resume_stop_20260828.md`, `Immortality System/07_Reports/immortality_system_libra_pilot_title_provider_stop_20260828.md`, `Immortality System/07_Reports/immortality_system_libra_pilot_manual_glossary_stop_20260828.md`, and `Immortality System/07_Reports/immortality_system_libra_pilot_term_provider_stop_20260828.md`. Do not start production `ch001-ch060` until the 20-chapter Pilot Gate completes and recommends a safe mode.
...
- V6.34 Milestone 6.2 DSE OOS stopped before valid measurement: isolated run `v6-34-m6-dse-oos-v1` completed 28 blocks through `ch088`, but final assembly exposed stale/off-by-one experiment raw source. `scripts/verify_experiment_source_parity.py` now reports `10/10` mismatches for the locked DSE OOS chapters when run from repo root against current production raw. Treat this vault's partial output as invalid measurement data. Report: `07_Reports/v6_34_m6_dse_oos_source_parity_stop_20260701.md`; research log: `01_Research_Log/2026-07-01_novel_pipeline_v6_34_m6_dse_oos_source_parity_stop.md`.
- V6.34 Milestone 6.3 DSE `ch029-block-005` treatment implemented: DSE OOS v2 was rebuilt with source parity `0` and no new OOS glossary approvals, then stopped on Chinese annotation leakage in an author promo (`[走进不科学]`). Added narrow output-side source-script annotation cleanup; tests passed; rerun from refine passed QA retry 2; `ch029` output exists and refined/formatted/final output contain no Han Chinese. Report: `07_Reports/v6_34_m6_dse_oos_ch029_source_script_treatment_20260701.md`; research log: `01_Research_Log/2026-07-01_novel_pipeline_v6_34_m6_dse_ch029_source_script_treatment.md`.
- V6.34 Milestone 6.2 DSE OOS completed validly: `v6-34-m6-dse-oos-v2` completed all 10 locked DSE OOS chapters (`55/55` blocks), current failed blocks none, manual actions none, source parity `0`, output guardrails passed, and final outputs exist in the isolated experiment vault. Final verification found a provider-hallucinated duplicate title-like body paragraph in `ch174`; final assembly now removes standalone title-like paragraphs anywhere in the body when the H1 title is authoritative. Report: `07_Reports/v6_34_m6_dse_oos_completion_20260701.md`; research log: `01_Research_Log/2026-07-01_novel_pipeline_v6_34_m6_dse_oos_completion.md`.
- V6.34 Milestone 6.2 IRS OOS completed validly: isolated run `v6-34-m6-irs-oos-v1` completed all 10 locked OOS chapters (`ch012`, `ch053`, `ch095`, `ch144`, `ch187`, `ch208`, `ch258`, `ch290`, `ch323`, `ch372`) with `33/33` blocks complete, current failed blocks none, manual actions none, source parity `0`, deterministic experiment-output checks passed, and latest scoped Sentinel `0/0/0/0` for every chapter. Smoothness remains imperfect: 2 OpenRouter empty-assistant refining failures recovered, QA used Gemini Flash fallback 3 times, and formatting used local fallback 4 times. Report: `07_Reports/v6_34_m6_irs_oos_completion_20260701.md`; research log: `01_Research_Log/2026-07-01_novel_pipeline_v6_34_m6_irs_oos_completion.md`.
- V6.34 Milestone 6 cross-novel OOS comparison completed: HGD, DSE, and IRS final OOS runs all ended with current failed blocks `0`, source parity `0`, deterministic output issues `0`, and latest Sentinel blocker/major `0/0`. Recommendation: continue production only as bounded sequential batches with scan/glossary gates, blocking Sentinel, deterministic output guardrails, and major-run spot-checks. Broad unattended parallel translate/refine/QA is not recommended yet because OOS still showed provider failures and recovery loops. Report: `07_Reports/v6_34_m6_cross_novel_oos_comparison_20260701.md`; research log: `01_Research_Log/2026-07-01_novel_pipeline_v6_34_m6_cross_novel_oos_comparison.md`.
...
| Experiment vault copied from stale source cache | before treatment/OOS provider calls, run `scripts/verify_experiment_source_parity.py` against the exact sampled chapters and require zero mismatches; rebuild the vault from current production `03_Raw`, title sidecars, and manifest if parity fails |
...
1. Immortality System: keep `immortality-libra-v1-insample` stopped at the `錢雅` provider failure in `ch376`; restore a usable term-suggestion route and run the exact health probe, then resume the same run. Seven sample chapters/29 blocks are complete in experiment-only output, title fallback behavior is implemented/tested, and no production or MoonRead content changed. Complete the Pilot Gate before production.
