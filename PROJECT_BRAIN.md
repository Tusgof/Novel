...
- `ch001-ch006` are translated and independently verified for MoonRead publication. `ch005` is live at pushed commit `ef37c463c70ea878eabdef35237031e8da004028`; `ch006` has passed local publication gates and awaits this session's push/deployment verification.
...
Active scope is V6.35 Re:Zero `ch005-ch010`. Re:Zero `ch006` has passed local publication gates; `ch007` is active under a chapter-isolated worker. Runs `ch008-ch010` are glossary-approved and have not started body translation.

1. Push and verify the `ch006` MoonRead publication without including protected unrelated dirty WIP.
2. Finish and independently accept `ch007`, then reuse the two worker slots for `ch008/ch009` followed by `ch010`.
