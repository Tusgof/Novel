...
- Production is explicitly approved for `ch001-ch150` in sequential five-chapter bounded batches. `ch001-ch010` are complete with no current failed blocks or manual actions. Run `immortality-system-ch011-ch015-v1` completed 20/20 blocks and final deterministic guardrails plus scoped Sentinel passed `0/0/0/0`, but work is stopped before `ch016` because the ledger records a recovered OpenRouter DeepSeek V4 Flash empty-assistant refinement failure at `ch014-block-002`; the work order requires stopping on any provider failure. Spot-check also found malformed verb-context renderings of bare `築基` in `ch011-ch012`. Evidence and repair plan: `Immortality System/07_Reports/immortality_system_ch011_ch015_production_stop_20260829.md`.
...
1. Immortality System: production `ch001-ch150` is approved and complete through `ch015`, but stopped on the recovered provider failure recorded at `ch014-block-002`. On explicit resume, first repair the bare `築基` verb policy and rerun affected `ch011-ch012` blocks with full gates; only then open the fresh `ch016-ch020` scan-only/glossary gate. Do not copy pilot artifacts into production.
