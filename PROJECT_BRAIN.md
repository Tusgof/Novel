...
1. Immortality System: keep `immortality-libra-v1-insample` stopped at `ch1765` glossary approval until the exact provider health checks pass and the fallback authentication issue is resolved or explicitly rerouted; title fallback behavior is now implemented/tested and all 20 sample title sidecars validate. Then resume the same bounded run and complete the Pilot Gate before production.
