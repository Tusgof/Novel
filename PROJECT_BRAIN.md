...
- `ch001-ch008` are translated, independently verified, and live on MoonRead. `ch008` completed `10/10` blocks with no current failure or manual action, passed output guardrails, runtime Sentinel `0/0/0/0`, Inspector Sentinel `0/0/8/0` with source-backed references only, spot-check, and MoonRead publication gates. Commit `5e3fbfc303930711202465a3408321d66de35a43` is live and its production URL returned HTTP `200` with the Thai title. Checkpoint: `Re Zero Watching Him Die Again and Again/07_Reports/rezero_ch008_production_checkpoint_20260917.md`.
...
- Run `rezero-ch006-production-v1` completed `10/10` blocks with no current failure or manual action. Output guardrails passed; Inspector Sentinel was `0/0/1/0` with only source-backed `Styx Helix`; Inspector spot-check passed. Block `007` was rerun from translate after QA exposed broad defects, and the replacement passed without force-accept. Checkpoint: `Re Zero Watching Him Die Again and Again/07_Reports/rezero_ch006_production_checkpoint_20260917.md`.
- Run `rezero-ch007-production-v1` completed `9/9` blocks with no current failure or manual action. Output guardrails passed; Inspector Sentinel was `0/0/4/0` with source-backed references only; Inspector spot-check and MoonRead publication gates passed. Blocks `006-008` received narrow source-backed recovery without force-accept. Checkpoint: `Re Zero Watching Him Die Again and Again/07_Reports/rezero_ch007_production_checkpoint_20260917.md`.
- Runs `rezero-ch009-production-v1` and `rezero-ch010-production-v1` stopped safely on provider exhaustion. `ch009` is `5/12` with block `006` failed at `translate`; `ch010` is `1/19` with block `002` failed at `qa`. Neither has a final output or MoonRead publication. OpenRouter lacks enough credit for the configured 12,000-token route, and the Codex fallback CLI session is expired. Checkpoint: `Re Zero Watching Him Die Again and Again/07_Reports/rezero_ch009_ch010_provider_limit_checkpoint_20260917.md`.
