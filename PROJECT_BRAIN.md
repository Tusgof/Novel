...
1. Immortality System: production `ch001-ch150` is approved but currently stopped with `ch011-block-004` pending post-refinement QA after three bounded resume attempts ended on OpenRouter reasoning empty-assistant responses. Resume only after provider state changes or an explicitly approved QA routing decision; then verify/assemble `ch011`, rerun `ch012-block-004` from refine, and pass both chapters through guardrails/Sentinel before opening `ch016-ch020`. Do not copy pilot artifacts into production.
